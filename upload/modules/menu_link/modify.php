...
// get target page_id
$sql_result     = $backend->db()->query("SELECT * FROM `:prefix:mod_menu_link` WHERE section_id = '$section_id'");
...
$query_page = $backend->db()->query("SELECT `target` FROM `:prefix:pages` WHERE `page_id` = '$page_id'");
$page       = $query_page->fetch(PDO::FETCH_ASSOC);
$target     = $page['target'];
